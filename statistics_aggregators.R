...
statistics.pivot<-function(VAR.Path
    ...
    competition.DF<-apply_lookups(Path,competition.DF)
    
    if(!is.na(UnlabeledValue)){
        competition.DF<-subset(competition.DF
                               ,Competition.sum!=UnlabeledValue
        )
    ...
    for(f in unique(competition.DF[,VAR.Attribute])){
        ...
        competition.combined[,paste("p",f,sep="")]<-competition.combined$p
        competition.combined<-subset(competition.combined,select=-c(p))
    }
...
fixed.price.statistics<-function(Path
    ...
    fixed.price.DF<-apply_lookups(Path,fixed.price.DF)
