...
```
**Observations** EnC is not significant, deos not affect anything meaningfully will not be included. 
...
```
**Observations:** IDV is statistically significant at the 95% level. It increased adjusted r^2 by .07. Will be included in the model.
...
modelh <- lm(pEffComp ~ pPSRServices+ I(pPSRServices^2)+ pPlatWnA+ pIDV+ pPlatAir+ pPaltMnS, data = MCClist)
summary(modelh)
...
```
**Observations:** Missiles and Space platform is not statisticaly significant, does not change other cofeficients and does not affect r^2 (except decreases by .01), MnS will not be included
...
```
**Observations:** Other platform does is not significant, doesn't affect r^2 and increases doesn't affect other coefficients in the model by very much and Will not be included in the model. 
...
```
**Observations** RnD is not statistically significant, does not change r^2 or adjusted and doesn't affect other variables in the model, RnD will not be included 
...
```
**Observations** IDV MAC is not statistically significant, it does not change r^2, it does not affect the other  veriables in the model in a meaningful way, . It will not be included in the model. 
...
```
**Observations** IDV SAC is statistically significant (90%), increases adjusted r^2 by .03 and  will be included in the model. 
...
```
**Observations** PSC land is not statistically significant, It decreases adjusted r^2 by .02, and due to its scatterplot, it will not be included in the model. The scatterplot has no indication of a statistically significant realionship. 
...
```
**Observations** MnS is not statistically significant, does not affect the other variables in a meaningful way and doesnt affect adjusted r^2, psc MnS will not be included in the model. Also it's scatter plot suggests this is all true. 
...
```
**Observations:** FRSnC does not change the other aspects of the model by very much, WnA decreases by .06, EnC by .02, Air by .02 and its does not affect r^2 or adjusted r^2 by mroe than .01, it will not be included in the model. 
...
``` {r echo=TRUE}
zfit <- lm(pEffComp ~ pPSRServices+ pPlatWnA+ pPlatEnC+ pIDV+ pPlatLand+ pPlatAir+ pPaltMnS+ pPlatVessel+ pPlatOther+ pRnD+ pPriceFixed+ pVehMAC+ pVehSAC+ pPscAir+ pPscLand+ pPscMnS+ pPscEngines+ pPscShips+ pPscFRSnC+ pPscPAMS+ pPscOther, data = MCClist)

step(zfit)

AIC(zfit)

afit <- lm(formula = pEffComp ~ pPSRServices + pPlatWnA + pPlatEnC + pPlatLand + pPaltMnS + pPlatVessel + pPlatOther + pRnD + pPriceFixed + pVehMAC + pVehSAC + pPscAir + pPscLand + pPscShips, data = MCClist)
