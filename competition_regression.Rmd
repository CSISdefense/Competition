...
afit <- lm(pEffectiveComp ~ pcService+ I(pcService^2)+ pRnD+ sqrt(pRnD)+ pAir, data=MCClist)
bfit <- lm(pEffectiveComp ~ pcService+I(pcService^2)+ pRnD+ sqrt(pRnD)+ pAir+sqrt(pAir), data=MCClist)  
fit2 <- lm(pEffectiveComp ~ pcService+ I(pcService^2)+ pRnD+ sqrt(pRnD)+ pAir+ sqrt(pAir) + pVessel, data=MCClist)
fit8 <- lm(pEffectiveComp ~ pcService+ I(pcService^2)+ pRnD+ sqrt(pRnD)+ pAir+ sqrt(pAir) + pVessel+ sqrt(pVessel), data=MCClist)
fit4 <- lm(pEffectiveComp ~ pcService+ I(pcService^2)+ pRnD+ sqrt(pRnD)+ pAir+ sqrt(pAir) + pVessel+ sqrt(pVessel), data=MCClist[rownames(MCClist)!=58,])
screenreg(list(afit, bfit, fit2, fit8, fit4))
...
bfit <- lm(pEffectiveComp ~ pcService+I(pcService^2)+ pRnD+ sqrt(pRnD)+ pAir+sqrt(pAir), data=MCClist)  
afit1 <- lm(pEffectiveComp ~ pcService+I(pcService^2)+ pRnD+ sqrt(pRnD)+ pAir + sqrt(pAir)+ pLand, data=MCClist)
summary(afit1) 
...
##doesn't change descriptive statistics 
```
...
bfit1 <- lm(pEffectiveComp ~ pcService+I(pcService^2)+ pRnD+ sqrt(pRnD)+ pAir + sqrt(pAir)+ pMnS, data=MCClist)
summary(bfit1) 

bfit2 <- lm(pEffectiveComp ~ pcService+I(pcService^2)+ pAir + pMnS, data=MCClist)
summary(bfit2) 
plot(bfit2)
screenreg(list(bfit1, bfit2))
...
bfit4 <- lm(pEffectiveComp ~ pcService+I(pcService^2)+ pAir + pMnS  +pVessel, data=subset(MCClist,                                                                                    rownames(MCClist)!=58))
summary(bfit4)

screenreg(list(bfit, bfit1, bfit2, bfit3, bfit4))
##pmns is not significant and doesn't change anything significantly 
```

remove pRnD (p=.06), sqrt(pRnD) (p=.08) and sqrt(pAir)(p=.34) pMns (p=.07) vessel (p=.67) as well as high p-values, these variables don't change the descriptive statistics of the other variables in a statistical or meaningful way. (SRC) 

Due to the correlation betwen pMnS and pRnD, it risks multicollinearity to have them both in the same plot.Happily, MnS appears has the correct sign and a lower p-value than sqrt(RnD) which also eliminates the difficult to interpret square roots. Trading them appears to be a positive exchange. Land briefly showed the wrong sign, but upon removing entry 11, that problem was eliminated. However, adding vessel back in without entry 58 still leaves vessel with a positive sign.  **NOTE** since the p-value of pMns is not significant and that neither the coefficients on service^2 OR the other variables change significantly pMns should not further be included. 
...
screenreg(list(cfit1, cfit2, cfit3))
...
dfit1 <- lm(pEffectiveComp ~ I(pcService^2)+ pAir +pWnA +pEnC, data=MCClist)
summary(dfit1) 
plot(dfit1)

dfit2 <- lm(pEffectiveComp ~ I(pcService^2)+ pAir +pWnA +pLand+ pEnC, data=MCClist)
summary(dfit2) 
plot(dfit2)
screenreg(list(dfit1, dfit2))

dfit3 <- lm(pEffectiveComp ~ I(pcService^2)+ pAir +pWnA +pEnC +pVessel, data=subset(MCClist,
...
efit1 <- lm(pEffectiveComp ~ I(pcService^2)+ pAir  +pMnS +pWnA +pEnC +pFnC, data=MCClist)
summary(efit1) 
plot(efit1)

efit2 <- lm(pEffectiveComp ~ pcService+I(pcService^2)+ pAir  +pMnS +pWnA +pEnC +pFnC +pLand, data=MCClist)
summary(efit2) 
plot(efit2)
screenreg(list(efit1, efit2))

efit3 <- lm(pEffectiveComp ~ I(pcService^2)+ pAir  +pMnS +pWnA +pEnC +pVessel + pFnC, data=MCClist)
summary(efit3) 
plot(efit3)
screenreg(list(efit1, efit2, efit3))

efit4 <- lm(pEffectiveComp ~ I(pcService^2)+ pAir  +pMnS +pWnA +pEnC +pVessel + pFnC, data=subset(MCClist,
                                                                                         rownames(MCClist)!=58 &
                                                                                             rownames(MCClist)!=59 ))
summary(efit4) 
plot(efit4)
...
ffit1 <- lm(pEffectiveComp ~ I(pcService^2)+ pAir  +pMnS +pWnA +pEnC +pFnC +pIDV, data=MCClist)
summary(ffit1) 
plot(ffit1)

ffit2 <- lm(pEffectiveComp ~ I(pcService^2)+ pAir + +pMnS + pWnA +pEnC +pIDV +pFnC +pService, data=MCClist)
...
gfit1 <- lm(pEffectiveComp ~ I(pcService^2)+ pAir + pEnC +pIDV +pWnA +pFnC +pFixed, data=MCClist)
summary(gfit1) 
dfMCC<-rbind(dfMCC,LMsummaryLine(gfit1))
plot(gfit1)
screenreg(list(gfit1))
...
```{r CostPluse}
ggplot(data=MCClist,aes(x=pFixed,y=pEffectiveComp))+geom_point()
hfit1 <- lm(pEffectiveComp ~ I(pcService^2)+ pAir + pEnC +pWnA +pFnC+pIDV+pCostPlus, data=MCClist)
summary(hfit1) 
dfMCC<-rbind(dfMCC,LMsummaryLine(hfit1))
plot(hfit1)

hfit2 <- lm(pEffectiveComp ~ I(pcService^2)+ pAir + pEnC +pWnA +pFnC+pIDV+pCostPlus, data=subset(MCClist,
...
ifit1 <- lm(pEffectiveComp ~ I(pcService^2)+ pAir  +pMnS +pWnA +pEnC +pFnC +pIDV +pIncentFee, data=MCClist)
summary(ifit1) 
plot(ifit1)

ifit2 <- lm(pEffectiveComp ~ I(pcService^2)+ pAir  +pWnA +pEnC +pFnC +pIDV +pCostPlus +pIncentFee+pCostPlus:pIncentFee, data=MCClist)
summary(ifit2) 
plot(ifit2)
...
jfit1 <- lm(pEffectiveComp ~ I(pcService^2)+ pAir + pMnS  +pWnA+pEnC +pFnC +pIDV+log(TotalCount), data=MCClist)
summary(jfit1)
plot(jfit1)
...
ggplot(data=MCClist,aes(x=TotalCount,y=pEffectiveComp))+geom_point()
ggplot(data=MCClist,aes(x=log(TotalCount),y=pEffectiveComp))+geom_point()
jfit2 <- lm(pEffectiveComp ~ I(pcService^2)+ pAir + pMnS  +pWnA+pEnC +pFnC +pIDV+log(TotalCount)+pLand+pLand:I(pcService^2), data=MCClist)
summary(jfit2)
plot(jfit2)
